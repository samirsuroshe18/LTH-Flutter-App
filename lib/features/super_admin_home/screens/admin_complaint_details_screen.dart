...
class _AdminComplaintDetailsScreenState extends State<AdminComplaintDetailsScreen> {
  ...
  void initState() {
    ...
    if(widget.data!=null){
      ...
      context.read<SuperAdminHomeBloc>().add(GetComplaintDetails(id: widget.data!.id!));
    }else{
      context.read<SuperAdminHomeBloc>().add(GetComplaintDetails(id: widget.complaintId!));
    }
  ...
  void _navigateAndSelectTechnician() async {
    final selected = await Navigator.pushNamed(context, '/tech-selection-screen', arguments: widget.data);
  ...
  void _blocListener(BuildContext context, SuperAdminHomeState state) {
    ...
    if (state is GetComplaintDetailsLoading) {
      _isLoading = true;
    }

    if (state is GetComplaintDetailsSuccess) {
      _isLoading = false;
      complaintModel = state.response;
      resolutionStatus = complaintModel?.resolution?.status ?? 'pending';
      setState(() {
    ...
    if (state is GetComplaintDetailsFailure) {
      complaintModel = null;
      _isLoading = false;
