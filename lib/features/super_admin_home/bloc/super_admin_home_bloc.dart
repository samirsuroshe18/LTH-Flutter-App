...
class SuperAdminHomeBloc extends Bloc<SuperAdminHomeEvent, SuperAdminHomeState> {
  ...
  SuperAdminHomeBloc({required SuperAdminHomeRepository superAdminHomeRepository})
    ...
    on<AddSectorAdmin>((event, emit) async {
      ...
      try{
        final Sectoradmin response = await _superAdminHomeRepository.addSectorAdmin(userName: event.userName, email: event.email, phoneNo: event.phoneNo, password: event.password, sectorType: event.sectorType);
